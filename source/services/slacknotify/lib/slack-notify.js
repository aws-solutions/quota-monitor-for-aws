  ...
  slackMessageBuilder(event) {
    ...
    let _slackMssg = {
      attachments: [{
        ...
        "title_link": "https://aws.amazon.com/answers/account-management/limit-monitor/",
        "footer": "Take Action?",
        "actions": [{
          "text": "AWS Console",
          "type": "button",
  ...
  processEvent(slackURL, slackMssg, cb) {
    ...
    _self.postMessage(slackURL, slackMssg, (response) => {
      if (response.statusCode < 400) {
        ...
        LOGGER.log('WARN', `Error posting message to Slack API: ${response.statusCode} - ${response.statusMessage}`);
        return cb(response.statusMessage, null); // Don't retry because the error is due to a problem with the request
      } else {
        // Let Lambda retry
        return cb(`Server error when processing message: ${response.statusCode} - ${response.statusMessage}`);
      }
    });
  }
